...
// variables and constructor are declared at the end
	...
	*/
	function load_strings() {
		...
		// dynamic
		if( file_exists( get_stylesheet_directory() . '/subscribe2-post/strings.php' ) ) {
			include_once( get_stylesheet_directory() . '/subscribe2-post/strings.php' );
...
/* ===== mail handling ===== */
	...
	*/
	function headers($type = 'text', $attachments = array()) {
		if ( empty($this->myname) || empty($this->myemail) ) {
			...
			/**
			 * Implements CUSTOM sender and sender name
			 * Overrides existing settings for the 'sender' option
			 * 
			 * Format: sender@email OR my nasty name <sender@email>
			 * @author Fabian Wolf
			 */
			if( !empty( $this->subscribe2_options['custom_sender'] ) ) {
				
				
				// its NOT the "my name <sender@email>" format
				if( strpos( $this->subscribe2_options['custom_sender'], '<' ) === false ) {
					// use blogname as fallback for the sender name
					$this->myname = html_entity_decode(get_option('blogname'), ENT_QUOTES);
					
				} else { // split em up
					$arrSender = explode('<', $this->subscribe2_options['custom_sender']);
					
					$this->myname = trim($arrSender[0]);
					$this->myemail = trim( str_replace('>', '', $arrSender[1] ) );
	...
	*/
	function publish($post, $preview = '') {
		...
		// do the keyword replacements beforehand
		$mailtext = $this->fill_keywords( $mailtext, array( 'html_header' => $this->subscribe2_options['html_header_template'] ) );
		...
		if ( $preview != '' ) {
			...
			// defaults to html
			$public_mail_type = 'html';
			
			switch( $this->subscribe2_options['custom_type_public_subs'] ) {
				default: // html_excerpt
				case 'html_excerpt':
					$public_mail_body = $html_excerpt_body;
					break;
				case 'html':
					$public_mail_body = $html_body;
					break;
				case 'plain_excerpt':
					$public_mail_type = 'text';
					$public_mail_body = $plain_excerpt_body;
					break;
				case 'plain':
					$public_mail_type = 'text';
					$public_mail_body = $plain_body;
					break;
			}
			
			$this->mail($recipients, $subject, $public_mail_body, $public_mail_type);
	...
	function insert_custom_keywords( $content ) {
		$return = $content;
	 ...
	 * NOTE: All keywords in the content are UPPER CASE, and surrounded by curly brackets, eg. {MY_KEYWORD}. Underscores "_" replaces space chars.
	 * 
	 * @since 9.1
	 * @author Fabian Wolf
	 * 
	 * @param string $content			The text we are going to parse.
	 * @param array $custom_keywords	An associative array, consisting of the keywords and their replacments. An additional array per keyword may be used for class method callbacks (both internally as well as externally; in the latter case only if the class name is provided).
	 * 
	 */
	 
	function fill_keywords( $content, $custom_keywords = array() ) {
		$return = $content;
		
		if( !empty( $custom_keywords ) && is_array( $custom_keywords) ) {
			foreach( $custom_keywords as $strTrigger => $uReplacement ) {
				$strTriggerKeyword = '{' . strtoupper( str_replace(' ', '_', $strTrigger ) ) . '}';
				
				if( strpos( $return, $strTriggerKeyword ) !== false ) { // keyword found at least once
					$return = str_replace( $strTriggerKeyword, $uReplacement, $return );
					
					/**
					 * NOTE: Callbacks are implemented later on
					if( !is_array($uReplacement) ) { // is a regular string, integer or anything else ..
						$return = str_replace(, $uReplacement, $return );
					} else { // ... is a callback!
						
						if( strpos( $uReplacement[0],'this' ) && method_exists( $this, $uReplacement[1]) ) {
							$return = str_replace( $strTriggerKeyword, $this->$uReplacement[1] );
						} else { // scheme: class::method()
							if( class_exists($uReplacement[0] ) && method_exists( $uReplacement[0], $uReplacement[1] ) || is_callable() ) {
							$return = str_replace( $strTriggerKeyword 
						}
					}
					*/
	...
	*/
	function send_confirm($what = '', $is_remind = false) {
		...
		if ( $is_remind == true ) {
			...
			// ACTION = what is TO DO ... NOT: a URL!
			if ( 'add' == $what ) {
		...
		// add HTML support
		if( stripos( $this->subscribe2_options['custom_type_public_subs'], 'html') !== false ) {
			
			// .. for the link
			$strLink = $link; // backup
			$link = str_replace($strLink, '<a href="' . $strLink . '">'.$strLink.'</a>', $link );
			
			
			// ... for the html header
			if( !empty($this->subscribe2_options['html_header_template']) ) {
				$strHTMLHeader = $this->subscribe2_options['html_header_template'];
			}
		}
		
		$strMailBody = $body;
		
		$body = $this->fill_keywords( $strMailBody, array('link' => $link, 'html_header' => $strHTMLHeader ) );
		
		
		
		//$body = str_replace( array("{HTML_HEADER}", "{LINK}", ), array($strHTMLHeader,$link), $body);
		...
		if ( $is_remind == true && function_exists('wpmq_mail') ) {
			...
			//__debug( array('type' => $mail_type, 'body' => $body), 'message' );
			
			
			return $this->mail( array($this->email), $subject, $body, $mail_type);
...
/* ===== Our constructor ===== */
	...
	*/
	function s2init() {
		...
		// add filter call for the html mail content
		//add_filter('s2_email_template', array( $this, 'insert_custom_keywords' ) );
