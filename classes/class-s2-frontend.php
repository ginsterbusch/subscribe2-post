...
/* ===== template and filter functions ===== */
	...
	*/
	function shortcode($atts) {
		...
		$unsubscribe_button_value = ( !empty( $unsubscribe_button_text ) ? $unsubscribe_button_text : apply_filters('s2_unsubscribe_button', __('Unsubscribe', 'subscribe2')) );
		$subscribe_button_value = ( !empty( $subscribe_button_text ) ? $subscribe_button_text : apply_filters('s2_subscribe_button', __('Subscribe', 'subscribe2')) );
		
		// add email label
		if( empty( $email_label ) ) {
			$email_label = __('Your email:', 'subscribe2');
		}
		...
		if ( $hide == 'subscribe' ) {
			$this->input_form_action = "<input type=\"submit\" class=\"$unsubscribe_button_class\" name=\"unsubscribe\" value=\"" . esc_attr($unsubscribe_button_value) . "\" />";
		} elseif ( $hide == 'unsubscribe' ) {
			$this->input_form_action = "<input type=\"submit\" class=\"$subscribe_button_class\" name=\"subscribe\" value=\"" . esc_attr($subscribe_button_value) . "\" />";
		} else {
			// both form input actions
			$this->input_form_action = "<input type=\"submit\" class=\"$subscribe_button_class\" name=\"subscribe\" value=\"" . esc_attr($subscribe_button_value) . "\" />&nbsp;<input type=\"submit\" class=\"$unsubscribe_button_class\" name=\"unsubscribe\" value=\"" . esc_attr($unsubscribe_button_value) . "\" />";
		}
		...
		if ( $noantispam != 'true' ) {
			...
			$antispam_text .= "<label for=\"uri\">Dont Change:</label><input type=\"text\" id=\"uri\" name=\"uri\" value=\"http://\" />";
			$antispam_text .= "</span>";
		}

		// build default form
		if ( $nojs == 'true' ) {
			$this->form = "<form method=\"post\"" . $action . "><input type=\"hidden\" name=\"ip\" value=\"" . $_SERVER['REMOTE_ADDR'] . "\" />" . $antispam_text . "<p><label for=\"s2email\">" . $email_label . "</label><input type=\"text\" name=\"email\" id=\"s2email\" value=\"" . $value . "\" size=\"" . $size . "\" />" . $wrap_text . $this->input_form_action . "</p></form>";
			
		} else {
			$this->form = "<form method=\"post\"" . $action . "><input type=\"hidden\" name=\"ip\" value=\"" . $_SERVER['REMOTE_ADDR'] . "\" />" . $antispam_text . "<p><label for=\"s2email\">" . $email_label . "</label><input type=\"text\" name=\"email\" id=\"s2email\" value=\"" . $value . "\" size=\"" . $size . "\" onfocus=\"if (this.value == '" . $value . "') {this.value = '';}\" onblur=\"if (this.value == '') {this.value = '" . $value . "';}\" />" . $wrap_text . $this->input_form_action . "</p></form>\r\n";
		}
	...
	*/
	function title_filter($title) {
		...
		if ( in_the_loop() ) {
			...
			//__debug( array('return' => $return, 'action' => $action ), 'result' );
		} 
	...
	* 
	* FIXME: $this->filtered fails if the_content is used more than once
	*/
	function confirm($content = '') {
		global $wpdb;

		//__debug('confirm fires!');
		...
		if ( '1' == $action ) {
			...
			//__debug( array('message' => $this->message, 'added' => $this->added), 'Subscribed!');
			...
			// fucks up, if content is called more than once
			//$this->filtered = 1;
		} elseif ( '0' == $action ) {
			...
			//__debug( array('message' => $this->message, 'deleted' => $this->deleted), 'UNsubscribed!');
		...
		if ( !empty($this->message) ) {
			//__debug( array('message' => $this->message), 'pre_return' );
			
			$return = '<div class="s2p-message">'. $this->message . '</div>';
