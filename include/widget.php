...
class S2_Form_widget extends WP_Widget {
	...
	*/
	function widget($args, $instance) {
		...
		// text
		$subscribe_button_text = empty($instance['subscribe_button_text']) ? '' : ' subscribe_button_text="' . $instance['subscribe_button_text'] . '"';
		$unsubscribe_button_text = empty($instance['unsubscribe_button_text']) ? '' : ' unsubscribe_button_text="' . $instance['unsubscribe_button_text'] . '"';
		
		// class
		$subscribe_button_class = empty($instance['subscribe_button_class']) ? '' : ' subscribe_button_class="' . $instance['subscribe_button_class'] . '"';
		$unsubscribe_button_class = empty($instance['unsubscribe_button_class']) ? '' : ' unsubscribe_button_class="' . $instance['unsubscribe_button_class'] . '"';
		
		// custom labels
		$email_label = empty( $instance['email_label'] ) ? '' : ' email_label="'. $instance['email_label'] . '"';
		...
		$shortcode = '[subscribe2' . $hide . $postid . $size . $nojs . $noantispam . $subscribe_button_text . $unsubscribe_button_text . $subscribe_button_class . $unsubscribe_button_class . $email_label . ']';
		echo $before_widget;
	...
	*/
	function update($new_instance, $old_instance) {
		...
		$instance['subscribe_button_text'] = stripslashes($new_instance['subscribe_button_text']);
		$instance['unsubscribe_button_text'] = stripslashes($new_instance['unsubscribe_button_text']);
		
		$instance['subscribe_button_class'] = stripslashes($new_instance['subscribe_button_class']);
		$instance['unsubscribe_button_class'] = stripslashes($new_instance['unsubscribe_button_class']);
		
		$instance['email_label'] = stripslashes($new_instance['email_label']);
	...
	*/
	function form($instance) {
		...
		$subscribe_button_text = htmlspecialchars($instance['subscribe_button_text'], ENT_QUOTES);
		$unsubscribe_button_text = htmlspecialchars($instance['unsubscribe_button_text'], ENT_QUOTES);
		
		$subscribe_button_class = htmlspecialchars($instance['subscribe_button_class'], ENT_QUOTES);
		$unsubscribe_button_class = htmlspecialchars($instance['unsubscribe_button_class'], ENT_QUOTES);
		
		$email_label = htmlspecialchars($instance['email_label'], ENT_QUOTES);
	...
	// buttons: text
		...
		echo "<p><label for=\"" . $this->get_field_id('subscribe_button_text') . "\">" . __('Subscribe Button Text', 'subscribe2') . ":\r\n";
		echo "<textarea class=\"widefat\" id=\"" . $this->get_field_id('subscribe_button_text') . "\" name=\"" . $this->get_field_name('subscribe_button_text') . "\" rows=\"2\" cols=\"25\">" . $subscribe_button_text . "</textarea></label></p>\r\n";
		
		// unsubscribe button text
		echo "<p><label for=\"" . $this->get_field_id('unsubscribe_button_text') . "\">" . __('Unsubscribe Button Text', 'unsubscribe2') . ":\r\n";
		echo "<textarea class=\"widefat\" id=\"" . $this->get_field_id('unsubscribe_button_text') . "\" name=\"" . $this->get_field_name('unsubscribe_button_text') . "\" rows=\"2\" cols=\"25\">" . $unsubscribe_button_text . "</textarea></label></p>\r\n";
		
	// buttons: classes
		
		// subscribe button class
		
		echo "<p><label for=\"" . $this->get_field_id('subscribe_button_class') . "\">" . __('Subscribe Button Class', 'subscribe2') . ":\r\n";
		echo '<input class="widefat" id="' . $this->get_field_id('subscribe_button_class') . '" name="' . $this->get_field_name('subscribe_button_class') . '" type="text" value="' . $subscribe_button_class . "\" /></label></p>\r\n";
		
		// unsubscribe button class
		echo '<p><label for="' . $this->get_field_id('unsubscribe_button_class') . '">' . __('Unsubscribe Button Class', 'unsubscribe2') . ":\r\n";
		echo '<input class="widefat" id="' . $this->get_field_id('unsubscribe_button_class') . '" name="' . $this->get_field_name('unsubscribe_button_class') . '" type="text" value="' . $unsubscribe_button_class . "\" /></label></p>\r\n";
		
		// email label
		echo '<p><label for="' . $this->get_field_id('email_label') . '">' . __('E-Mail label', 'unsubscribe2') . ":\r\n";
		echo '<input class="widefat" id="' . $this->get_field_id('email_label') . '" name="' . $this->get_field_name('email_label') . '" type="text" value="' . $email_label . "\" /></label></p>\r\n";
